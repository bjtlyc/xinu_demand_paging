...
nulluser()				/* babysit CPU when no one is home */
{
    ...
    //init frame mapint table array to unmapped, frm_tab
    init_frm();
    //init backing store mapping table  bsm_tab array
    init_bsm();

    //create page structrue and set cr3
    unsigned long temp = create_ps() ;
    kprintf("%x\n", temp);
    write_cr3(temp);
    ...
    // store page directory base address in proc pdbr
    proctab[currpid].pdbr = temp;
    //initialize fifo/aging queue
    fifoq.next = NULL;
    lastfifoq = &fifoq;
...
*/
    ...
    //initialize free memory list1 in paging mode
